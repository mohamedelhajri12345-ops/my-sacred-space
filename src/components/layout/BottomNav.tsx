...
function AthkarIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 3v18" />
      <path d="M5 6l7-3 7 3" />
...
const ITEMS = [
  ...
  { to: "/athkar", label: "الأذكار", icon: AthkarIcon },
  { to: "/prayer", label: "الصلاة", icon: IslamicPrayerIcon },
  { to: "/athkar/tasbih", label: "المسبحة", icon: TasbihIcon },
] as const;
          ...
          return (
            <li key={item.to} className="flex-1">
              <Link
                to={item.to}
                onClick={() => haptic("soft")}
                className="soothing-btn relative flex flex-col items-center gap-1 rounded-xl px-1.5 py-2 text-[10px] font-medium transition-all duration-300"
              >
                {active && (
                  <motion.div
                    layoutId="activeTab"
                ...
                <span className={cn(
                  "relative z-10 transition-colors duration-300 text-center leading-tight",
